...
lcd.drawText (93,13, "ACC", INVERS)
lcd.drawText (90,39, "WINCH", INVERS,SMLSIZE)
...
local function draw_locked_diffs()
    ...
    local difflock = getOutputValue(2)
    if difflock == 100 then
...
local winch = getOutputValue (4)
local winmix = getLogicalSwitchValue (1)
if winch >  0 then
lcd.drawText (93,52, "Out")
end
if winch < 0 then
lcd.drawText (93,52, "In")
end
if winch == 0 then
lcd.drawText (93,52, "Dis")
end
...
--Cruise Control
local function acc()
local accpos = getLogicalSwitchValue (3)
if accpos == true then
lcd.drawText (93,26, "ACT")
end
if accpos == false then
lcd.drawText (93,26, "OFF")
end
end
...
--TRANSMISSION
 -- Display L for low speed.  Change channel number line 403 and value line 404 to suit your model
    local channel_3_value = getOutputValue(3)
